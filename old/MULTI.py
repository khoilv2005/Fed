...
import threading  # Threading cho parallel training
from concurrent.futures import ThreadPoolExecutor  # ThreadPoolExecutor cho threading
...
CONFIG = {
    ...
    # Multiprocessing
    'use_multiprocessing': True,  # 👈 QUAN TRỌNG: BẬT LÊN
...
def train_federated(server, config, train_loaders=None):
    ...
    for round_idx in round_iter:
        ...
        if use_multiprocessing:
            # ✅ MULTIPROCESSING THẬT (không phải threading!)
            print("📝 Chế độ: MULTIPROCESSING - Clients chạy song song...")
            if train_loaders is None:
                raise ValueError("train_loaders cần thiết cho multiprocessing")
            ...
            round_result = train_round_multiprocessing(  # ← ĐỔI THÀNH MULTIPROCESSING!
            server=server,
            config=worker_config,
            train_loaders=train_loaders,  # ← THÊM DÒNG NÀY!
            device=device
            )
